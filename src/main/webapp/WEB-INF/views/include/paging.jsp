		...
		<c:forEach var="i" begin="${param.startPageNo}"
			...
			<c:choose>
				<c:when test="${i eq param.pageNo}">
					<li><a href="/community?page=${i}<c:if test="${param.option != null && param.text != null}" >&option=${param.option}&text=${param.text}</c:if><c:if test="${param.game_abb != null}">&game_abb=${param.game_abb}</c:if>" class="choice" style="background-color: #3ec5f0;">${i}</a></li>
				</c:when>
				<c:otherwise>
